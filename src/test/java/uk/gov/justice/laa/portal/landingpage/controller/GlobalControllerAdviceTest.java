...
class GlobalControllerAdviceTest {
    ...
    @Test
    void getUserAuditTableFlag_flag_off() {
        boolean result = controller.getUserAuditTableEnabledFlag();
        assertThat(result).isFalse();
    }

    @Test
    void getUserAuditTableFlag_flag_on() {
        ReflectionTestUtils.setField(controller, "enableUserAuditTable", true);
        boolean result = controller.getUserAuditTableEnabledFlag();
        assertThat(result).isTrue();
    }

    @Test
    void getActiveFirm_returnsNull_forApiEndpoints() {
        MockHttpServletRequest request = new MockHttpServletRequest();
        request.setRequestURI("/api/v1/claims/enrich");

        FirmDto result = controller.getActiveFirm(authentication, request);

        assertThat(result).isNull();
    }

    @Test
    void getCurrentUserProfile_returnsNull_forApiEndpoints() {
        MockHttpServletRequest request = new MockHttpServletRequest();
        request.setRequestURI("/api/v1/claims/enrich");

        CurrentUserDto result = controller.getCurrentUserProfile(authentication, request);

        assertThat(result).isNull();
    }

    @Test
    void isInternal_returnsFalse_forApiEndpoints() {
        MockHttpServletRequest request = new MockHttpServletRequest();
        request.setRequestURI("/api/v1/claims/enrich");

        boolean result = controller.isInternal(authentication, request);

        assertThat(result).isFalse();
    }

    @Test
    void isExternal_returnsFalse_forApiEndpoints() {
        MockHttpServletRequest request = new MockHttpServletRequest();
        request.setRequestURI("/api/v1/claims/enrich");

        boolean result = controller.isExternal(authentication, request);

        assertThat(result).isFalse();
    }

    @Test
    void isInternal_returnsFalse_whenUserProfileIsNull() {
        when(loginService.getCurrentProfile(authentication)).thenReturn(null);
    ...
    @Test
    void isExternal_returnsFalse_whenUserProfileIsNull() {
        when(loginService.getCurrentProfile(authentication)).thenReturn(null);
