...
class GlobalControllerAdviceTest {
    ...
    void getActiveFirm_single_firm_external() {
        ...
        EntraUser entraUser = EntraUser.builder().isMultiFirmUser(false).userProfiles(Set.of(userProfile)).build();
        when(loginService.getCurrentEntraUser(any())).thenReturn(entraUser);
        FirmDto firmDto = controller.getActiveFirm(authentication);
    ...
    void getActiveFirm_multi_firm_not_active_profile() {
        ...
        EntraUser entraUser = EntraUser.builder().isMultiFirmUser(true).userProfiles(Set.of(userProfile)).build();
        when(loginService.getCurrentEntraUser(any())).thenReturn(entraUser);
        FirmDto firmDto = controller.getActiveFirm(authentication);
    ...
    void getActiveFirm_multi_firm_multi_profiles() {
        ...
        EntraUser entraUser = EntraUser.builder().isMultiFirmUser(true).userProfiles(Set.of(userProfile1, userProfile2)).build();
        when(loginService.getCurrentEntraUser(any())).thenReturn(entraUser);
        FirmDto firmDto = controller.getActiveFirm(authentication);
