...
class LoginServiceTest {
    ...
    @Mock
    private GraphApiService graph;           // collaborators
    @Mock
    ...
    void createsSessionData_whenGraphReturnsValues() {
        ...
        var authToken = new OAuth2AuthenticationToken(principal,
                principal.getAuthorities(), "azure");

        var client = MockitoTestUtils.authorizedClient("token-123"); // tiny helper

        // ---- stub Graph responses ----
        when(graph.getAppRoleAssignments(anyString()))
                .thenReturn(List.of(new AppRoleAssignment()));
        when(graph.getUserAssignedApps(anyString()))
                .thenReturn(List.of(new AppRole()));
        when(graph.getUserProfile(anyString()))
                .thenReturn(new User());
        when(graph.getLastSignInTime(anyString()))
                .thenReturn(LocalDateTime.of(2025, 4, 28, 12, 0));
        when(userService.getManagedAppRegistrations())
                .thenReturn(List.of());
