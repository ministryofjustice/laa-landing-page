...
public class DoNothingTechServicesClient implements TechServicesClient {
    ...
    @Override
    public TechServicesApiResponse<GetUserResponse> getUser(UUID entraOid) {
        logger.info("Get user request received on Dummy Tech Services Client for entra oid: {}",
                entraOid.toString());

        Optional<EntraUser> optionalDbUser = entraUserRepository.findByEntraOid(entraOid.toString());
