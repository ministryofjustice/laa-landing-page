...
public class RoleBasedAccessUserListTest extends RoleBasedAccessIntegrationTest {
    ...
    @Test
    public void testSecurityResponseCanSeeAllUsers() throws Exception {
    ...
    @Test
    public void testSecurityResponseCanSeeAllUsersWithExternalUserManagerWhenFiltered() throws Exception {
        EntraUser loggedInUser = securityResponseUsers.getFirst();
        MvcResult result = this.mockMvc.perform(get("/admin/users?size=100&showFirmAdmins=true")
                        .with(userOauth2Login(loggedInUser)))
                .andExpect(status().isOk())
    ...
    @Test
    public void testSecurityResponseCanSeeOnlyMultiFirmUsersWhenFiltered() throws Exception {
        EntraUser loggedInUser = securityResponseUsers.getFirst();
        MvcResult result = this.mockMvc.perform(get("/admin/users?size=100&showMultiFirmUsers=true")
                        .with(userOauth2Login(loggedInUser)))
                .andExpect(status().isOk())
    ...
    @Test
    public void testSecurityResponseCanSeeOnlyMultiFirmUsersWhenUsingBackButtonAndFiltered() throws Exception {
        EntraUser loggedInUser = securityResponseUsers.getFirst();
        MockHttpSession testSession = new MockHttpSession();
        // Send an initial request for user list with filter applied.
        this.mockMvc.perform(get("/admin/users?size=100&showMultiFirmUsers=true")
